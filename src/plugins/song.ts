...
export async function fetchSongDetails(songId: number): Promise<Song> {
  ...
  return fetch(`https://maimai.lxns.net/api/v0/maimai/song/${adjustedId}`)
    ...
    .then((data) => ({
      ...
      difficulties: {
        standard: data.difficulties.standard
          ? data.difficulties.standard.map((difficulty: any) => ({
          ...
          : [],
        dx: data.difficulties.dx
          ? data.difficulties.dx.map((difficulty: any) => ({
              type: difficulty.type,
              level: difficulty.level,
              level_value: difficulty.level_value,
              note_designer: difficulty.note_designer,
